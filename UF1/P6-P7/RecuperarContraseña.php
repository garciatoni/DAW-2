    ...
    if (isset($_REQUEST["E-mail"])){
        ...
        if (isset($_REQUEST["E-mail"]) && empty($_REQUEST["E-mail"])){
            ...
            if ($resultado->num_rows > 0){
                ...
                $id_user = idUsuario($_REQUEST["E-mail"]);
                $sql = "SELECT `id_user` FROM tokens WHERE id_user = $id_user";
                if (!$resultado = $conn->query($sql)){
                    die("Error ejuctuando la consulta:".$conn->error);
                }
                if ($resultado->num_rows > 0){
                    $sql = "UPDATE `tokens` SET `token` = '$token', `fecha_creacion` = '$fecha_actual' WHERE `tokens`.`id_user` = $id_user";
                    if (!$resultado = $conn->query($sql)){
                        die("Error ejuctuando la consulta:".$conn->error);
                    }
                }else{
                    $sql = "INSERT INTO `tokens` (`id_user`, `token`, `fecha_creacion`) VALUES ('$id_user', '$token', $fecha_actual)";
                    if (!$resultado = $conn->query($sql)){
                        die("Error ejuctuando la consulta:".$conn->error);
                    }
                }
