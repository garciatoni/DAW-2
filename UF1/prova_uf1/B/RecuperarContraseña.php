...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_REQUEST["E-mail"])){
        ...
        if (isset($_REQUEST["E-mail"]) && empty($_REQUEST["E-mail"])){
            ...
            $sql = "SELECT correo FROM usuaris_examen where correo='".$_REQUEST["E-mail"]."'";
            if (!$resultado = $conn->query($sql)){
                    die("Error ejuctuando la consulta:".$conn->error);
                }
            if ($resultado->num_rows > 0){
                $token = generarCodigo(50);
                $sql = "UPDATE `usuaris_examen` SET `token` = '$token' WHERE `usuaris_examen`.`correo` ='".$_REQUEST["E-mail"]."'";
                if (!$resultado = $conn->query($sql)){
                    die("Error ejuctuando la consulta:".$conn->error);
                }
                include ("Email.php"); 
